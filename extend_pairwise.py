...
with open(pairwise_tsv) as ORIGINAL, open(new_pairwise_tsv, 'w') as NEW:
    ...
    for or_line in tqdm(ORIGINAL, total=metadata_dict["edges"]):
        ...
        ani_1_in_2 = to_ani(containment_1_in_2, kSize, metadata_dict["scale"], n_unique_kmers= id_to_kmer_count[id_2]*metadata_dict["scale"]).ani
        containment_2_in_1 = shared_kmers / id_to_kmer_count[id_1]
        ani_2_in_1 = to_ani(containment_2_in_1, kSize, metadata_dict["scale"], n_unique_kmers= id_to_kmer_count[id_1]*metadata_dict["scale"]).ani
